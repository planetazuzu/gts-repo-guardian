...
export async function generateReadme(owner: string, repo: string, templateType: string = 'tech') {
  try {
    ...
    let template = TEMPLATES[templateType as keyof typeof TEMPLATES] || TEMPLATES.tech;
...
function detectStack(files: string[], description: string): string {
  const stack: string[] = [];
  ...
  if (files.includes('package.json')) {
    ...
    else if (files.includes('vite.config.ts')) stack.push('Vite');
    else if (files.includes('astro.config.mjs')) stack.push('Astro');
  }
  ...
  if (files.includes('pom.xml')) stack.push('Java');
  if (files.includes('Gemfile')) stack.push('Ruby');
  if (files.includes('composer.json')) stack.push('PHP');
  if (files.includes('angular.json')) stack.push('Angular');
  if (files.includes('ionic.config.json')) stack.push('Ionic');
  if (desc.includes('react')) stack.push('React');
  if (desc.includes('vue')) stack.push('Vue');
  if (desc.includes('typescript')) stack.push('TypeScript');
  if (desc.includes('python')) stack.push('Python');

  return stack.length > 0 ? stack.join(' · ') : 'JavaScript · Node.js';
}

function detectSanitario(description: string, files: string[]): boolean {
  ...
  const desc = description.toLowerCase();
  return keywords.some(k => desc.includes(k)) || files.some(f => 
    f.toLowerCase().includes('protocolo') || f.toLowerCase().includes('vademecum')
  );
}

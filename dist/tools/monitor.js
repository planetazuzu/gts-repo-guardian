...
export async function watchRepositories(username, daysInactive = 30) {
    try {
...
${inactiveRepos.length > 0 ? inactiveRepos.map(r => `- **${r.name}** - ${r.daysSinceUpdate} días sin actualizar (${r.updated})`).join('\n') : '✅ ¡Todos los repos están activos!'}
